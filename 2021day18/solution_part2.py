...
def makeSnail(number):
    return type("Snail",(object,),{"number" : copy.deepcopy(number),"_ptr" : [], # ptr is a list of [0,1,0,...]
                                   "_stack" : [[1],[0]], "_exploded" : False,"_split" : False})

def addSnail(snail0, snail1):
    return reduceSnail(makeSnail([copy.deepcopy(snail0.number), copy.deepcopy(snail1.number)]))
...
if __name__ == "__main__":
    ...
    indsToSum = list(itertools.permutations(np.arange(len(numbers)),2))
    sumValues = np.zeros(len(indsToSum),dtype=int)
    
    for ind, (ind0, ind1) in enumerate(indsToSum):
        print(str(ind) + "/" + str(len(indsToSum)))
        snail0 = makeSnail(numbers[ind0])
        snail1 = makeSnail(numbers[ind1])
        snail = addSnail(snail0, snail1)
        sumValues[ind] = magnitude(snail.number)

    print(np.max(sumValues))
    print(indsToSum[np.argmax(sumValues)])